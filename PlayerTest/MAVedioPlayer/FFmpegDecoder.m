...
- (int)ToPCM:(char *)dataBuf
{
    ...
    int outSize = av_samples_get_buffer_size(NULL, CHANNEL, len, AV_SAMPLE_FMT_S16, 0);
...
- (BOOL)ToRGB:(char *)outBuf andWithOutHeight:(int)outHeight andWithOutWidth:(int)outWidth
{
    ...
    pSwsCtx = sws_getCachedContext(pSwsCtx, pVideoCodecCtx->width, pVideoCodecCtx->height, pVideoCodecCtx->pix_fmt, outWidth, outHeight, AV_PIX_FMT_RGB24, SWS_BICUBIC, NULL, NULL, NULL);
    if (pSwsCtx) {
