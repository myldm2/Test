...
- (void)display:(NSData*)imageData width:(GLsizei)width height:(GLsizei)height
{
    ...
    unsigned int shaderProgram = [self setupProgram];
    glUseProgram(shaderProgram);
    
    unsigned int vao = [self setupVAO];
...
//    CGContextRelease(context);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, imageData.bytes);
...
- (void)setupOpenGLBuffers {
    ...
    [_eaglContext renderbufferStorage:GL_RENDERBUFFER fromDrawable:self.eaglLayer];
